...
folder are mapped to the application's root namespace, 'RtConfigurator'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "RtConfigurator"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'RtConfigurator' in the same way
as `"RtConfigurator/sass/src"`.
