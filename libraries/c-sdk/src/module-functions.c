...
int init(module_t* module, const uint8_t * data, uint32_t length)
{
    ...
    common_params_t* common_params = (common_params_t*) module->specific_params;
    ...
    ((common_params_t*)module->specific_params)->Task_Period = rt_timer_ns2ticks(bson_iter_int32(&iter) * 1000);
    ...
    ((common_params_t*)module->specific_params)->Transfer_task_period = rt_timer_ns2ticks(bson_iter_int32(&iter) * 1000);
...
void get_input_data(module_t *module)
{
    ...
    if(module->input_data==NULL)
    {
        ...
        rt_task_sleep(((common_params_t*)module->specific_params)->Task_Period);
    ...
    int res_read = rt_queue_read(&module->in_queue, buf, 256, ((common_params_t*)module->specific_params)->Task_Period);
    if (res_read > 0)
    {
        bson_t bson;
...
int transmit_object(module_t* module, RTIME* time_last_publish_shmem, bool to_queue)
{
    ...
    bool time2publish2shmem = (rt_timer_read() - *time_last_publish_shmem) > ((common_params_t*)module->specific_params)->Transfer_task_period;
    while(out_object)
    {
...
void task_transmit(void *p_module)
{
    ...
    while (1) {
        ...
        res = rt_cond_wait(&module->obj_cond, &module->mutex_obj_exchange, ((common_params_t*)module->specific_params)->Transfer_task_period);
...
int create_xenomai_services(module_t* module)
{
    ...
    int err = rt_task_create(&module->task_main, name_task_main, TASK_STKSZ, ((common_params_t*)module->specific_params)->Task_Priority, TASK_MODE);
    if (err != 0)
    {
